...
GirlSinger a = any.get(0); // Error...
```

```kotlin
// @Kotlin 버전 반공변 제한 리스트
...
// 어떤 그룹인지 알 수 없기에 함부로 걸그룹에 멤버를 넣을 수 없다
girls.add(new GirlsGeneration("태연"));
```
```kotlin
// @Kotlin
...
// 구체적인 그룹은 모르겠지만 걸그룹인건 알 수 있다
val aGirl:GirlSinger = ab.get(0)

// Error.
val aGirl:GirlsGeneration = ab.get(0) // 소녀시대의 요소가 있어도, 컴파일러는 알 수 없다
```
