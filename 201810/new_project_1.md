...
재사용성을 아무리 고려해도 실제 다양하게 사용되다보면 각자의 조금씩 다른 요구사항에 버려지는 케이스가 많기 때문이다. 그래서 설계가 상당히 중요하며, 아직도 감을 잡기 어렵다.

그래도 노력을 한 결과 몇몇 컴포넌트는 재사용성을 확보할 수 있었다. 디자인 요소를 배제할수록, 도메인 이해도가 높을 수록 재사용성이 높았던 것 같다.

- 각 팝업 버튼 재사용 (버튼을 컨테이너화해서 property 만으로 액션과 부수효과를 처리했다)
- 로직 UI 재사용 (전체취소, 확인요청 등의 비즈니스 덩어리를 정해진 프로퍼티만으로 표현할 수 있었다)
- PureComponent 를 최대한 다수를 생성하려고 했다. 재활용의 시작이 되니까.
- ...더 생각나면 적겠다.
